            ...
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row items-start justify-between gap-10 py-10 border-b border-slate-500/30 text-slate-500">
                    <div>
                        ...
                        <p className="max-w-[510px] mt-6 text-sm text-slate-600">As melhores ofertas em um só lugar. Na <span className="font-black bg-gradient-to-r from-indigo-500 via-fuchsia-500 via-30% via-rose-500 via-60% to-amber-500 bg-clip-text text-transparent animate-gradient">.PrismaOfertas</span>, reunimos produtos
                            de qualidade, variedade e preços competitivos para tornar sua experiência de compra mais simples, segura e inteligente.<br></br>
                            Nossa curadoria conecta você às melhores oportunidades do mercado, ajudando a encontrar o que procura pelo melhor custo-benefício. <br></br>
                            <span className="font-black bg-gradient-to-r from-indigo-500 via-fuchsia-500 via-30% via-rose-500 via-60% to-amber-500 bg-clip-text text-transparent animate-gradient">.PrismaOfertas</span> as 
                            melhores ofertas em um só lugar. Produtos selecionados, variedade e preços competitivos para você comprar melhor.
                        </p>
                    ...
                    {/* Container das seções — grid 2 colunas compacto no mobile, flex-wrap normal a partir do desktop */}
                    <div className="grid grid-cols-2 gap-x-4 gap-y-6 sm:flex sm:flex-wrap sm:justify-between sm:gap-5 w-full md:w-[45%] text-[11px] sm:text-sm">
                        {linkSections.map((section, index) => (
                            <div key={index} className={section.hiddenOnMobile ? "hidden sm:block" : ""}>
                                <h3 className="font-medium text-slate-900 text-xs sm:text-base mb-2 sm:mb-5">{section.title}</h3>
                                <ul className="space-y-1.5 sm:space-y-1.1">
                                    {section.links.map((link, i) => (
                                        <li key={i} className="flex items-center gap-1.5 sm:gap-2 text-slate-700">
                                            {link.icon && <link.icon />}
